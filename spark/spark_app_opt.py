...
logger.info(f"spark {spark.version}, cores={spark.sparkContext.defaultParallelism}, shuffle_partitions={NUM_PARTITIONS}, aqe=true")

total_start = time.time()

step_begin("load_and_cache")
...
cat_stats.persist(StorageLevel.MEMORY_ONLY)
cat_stats.show()
...
step_begin("top_products_with_broadcast")
prod_stats = (
...
result.orderBy(F.desc("revenue")).limit(20).show()
step_end("top_products_with_broadcast")
...
window_df = (
    df.withColumn("rank", F.rank().over(Window.partitionBy("category").orderBy(F.desc("total_amount"))))
    .persist(StorageLevel.MEMORY_AND_DISK)
)
window_df.filter(F.col("rank") <= 3).select("category", "product_name", "total_amount", "rank").orderBy("category", "rank").show(30)
